...
#// 外部声明
extern Entry symbolTable[HASH_SIZE];  // 声明一个哈希表用于符号表
extern Entry layersHead;               // 符号表的头节点

// 存储指令的双向链表的链表头节点
InterCode interCodes;

// 临时变量，标记的全局编号
int tmpVarNo = 1;  // 初始临时变量编号为1
int labelNo = 1;   // 初始标记编号为1

// 初始化双向链表
void initInterCodes() {
    // 这个函数的实现为空，因为全局变量 interCodes 会在编译时自动初始化为 NULL
}
...
InterCode translateExp(Node* root, Operand place) {
    ...
    if (root->childNum == 3 && strcmp(root->children[1]->name, "ASSIGNOP") == 0) {
        ...
        */
        if (root->children[0]->childNum == 1 && 
            ...
            // 创建一个临时变量，用于存储表达式的值
            Operand tmpVar = newTemp();
            // 翻译右侧表达式，结果存储在tmpVar中
            InterCode code1 = translateExp(root->children[2], tmpVar);

            // 创建一条赋值语句，将表达式的值赋给临时变量
            InterCode code2 = (InterCode)malloc(sizeof(InterCode_));
            code2->kind = ASSIGN_IR;
            code2->ops[0] = getVar(root->children[0]->children[0]->strVal);
            code2->ops[1] = tmpVar;
            code2->ops[2] = NULL;
    ...
    */
        ...
        // todo
        // 处理 NOT、RELOP、AND、OR 表达式
        Operand label1 = newLabel(); // 创建一个新的标签 label1 用于跳转到真值的地方
        Operand label2 = newLabel(); // 创建一个新的标签 label2 用于跳转到假值的地方
...
InterCode translateStmt(Node* root) {
    if (strcmp(root->children[0]->name, "Exp") == 0) {
        ...
        // 创建三个标签
        Operand label1 = newLabel();  // 循环起始标签
        Operand label2 = newLabel();  // 条件成立时跳转的标签
        Operand label3 = newLabel();  // 循环结束时的标签
        ...
        // 翻译while语句的条件表达式，生成条件判断的中间代码，使用label2和label3作为跳转目标
        ...
        // 优化循环体中的GOTO语句，将其目标标签替换为循环的起始标签
...
InterCode translateCond(Node* root, Operand labelTrue, Operand labelFalse) {
    // todo
    if (root == NULL) {
        return getNullInterCode(); // 空结点直接返回空中间代码
    }

    if (root->children[0] != NULL && strcmp(root->children[0]->name, "NOT") == 0) {
        // NOT 操作符的处理
        // 将真假标签颠倒，交换 labelTrue 和 labelFalse
        return translateCond(root->children[1], labelFalse, labelTrue);

    } else if (root->children[1] != NULL && strcmp(root->children[1]->name, "RELOP") == 0) {
        // 创建两个临时操作数
        Operand tmp1 = newTemp();
        Operand tmp2 = newTemp();

        // 翻译左操作数表达式，结果存储在 tmp1 中
        InterCode code1 = translateExp(root->children[0], tmp1);

        // 翻译右操作数表达式，结果存储在 tmp2 中
        InterCode code2 = translateExp(root->children[2], tmp2);

        // 创建条件跳转的中间代码，判断 tmp1 和 tmp2 是否满足关系
        InterCode code3 = (InterCode)malloc(sizeof(InterCode_));
        ...
        // 创建无条件跳转的中间代码，用于跳转到 labelFalse
        InterCode code4 = (InterCode)malloc(sizeof(InterCode_));
        code4->kind = GOTO_IR;
        code4->ops[0] = labelFalse;

        // 在条件检查之前生成函数调用和参数传递的代码
        insertInterCode(code2, code1);

        // 将条件跳转的中间代码插入到左操作数的中间代码之后
        insertInterCode(code3, code1);

        // 将无条件跳转的中间代码插入到左操作数的中间代码之后
        insertInterCode(code4, code1);

        // 返回中间代码
        return code1;

    } else if (root->children[1] != NULL && strcmp(root->children[1]->name, "AND") == 0) {
        // 生成新的标签
        Operand labelNext = newLabel();

        // 翻译左操作数，如果为真则跳转到 labelNext，否则跳转到 labelFalse
        InterCode code1 = translateCond(root->children[0], labelNext, labelFalse);

        // 翻译右操作数，如果为真则跳转到 labelTrue，否则跳转到 labelFalse
        InterCode code3 = translateCond(root->children[2], labelTrue, labelFalse);

        // 生成标签的中间代码
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_));
        code2->kind = LABEL_IR; // 标签
        code2->ops[0] = labelNext;

        // 插入标签中间代码到生成的中间代码序列中
        insertInterCode(code2, code1);

        // 插入右操作数的中间代码到生成的中间代码序列中
        insertInterCode(code3, code1);

        // 返回生成的中间代码序列
        return code1;

    } else if (root->children[1] != NULL && strcmp(root->children[1]->name, "OR") == 0) {
        // 生成一个新的标签，用于标记下一步跳转的位置
        Operand labelNext = newLabel(); 
        // 翻译左操作数条件
        InterCode code1 = translateCond(root->children[0], labelTrue, labelNext);
        // 翻译右操作数条件
        ...
        // 其他情况的处理，可能是基本类型的条件判断
        Operand tmp1 = newTemp(); // 临时变量
        InterCode code1 = translateExp(root, tmp1); // 翻译表达式

        // 根据表达式的值进行条件跳转
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_));
        code2->kind = IF_GOTO_IR; // 条件跳转
        code2->ops[0] = tmp1;
